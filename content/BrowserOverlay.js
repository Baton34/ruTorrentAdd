...
XULRuTorrentAddonChrome.BrowserOverlay = {
  ...
  openRuTorrentPage: function(RTMainUrl, rtUser, rtPasswd)
  {
    ...
    let numTabs = gBrowser.visibleTabs.length;  //.mPanelContainer.childNodes.length;
...
{   
    ...
    {        
        if (topic=='http-on-examine-response')
        {
            ...
            {					                                
				if (subject.URI.prePath==this.rt_uri.prePath && (subject.URI.path.indexOf(this.rt_uri.path)>=0 || subject.URI.path.indexOf("/RPC2")>=0))
                {
    ...
    register: function(rt_url, rt_user, rt_pass) 
    {
      ...
      let authentication = Cc["@mozilla.org/network/http-auth-manager;1"].
