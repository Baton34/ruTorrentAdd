...
XULRuTorrentAddon.AddingTorrentDialog = 
{
  ...
  {    
    ...
    {    
      ...
      serverLabel.value=stringsBundle.getString("rtadd.server")+" "+window.arguments[0].inn.rt_name+
                          "("+XULRuTorrentAddon.rtURL+")";
    }
    else
    {
...
//                                           };
  ...
  showDirectoryDialogWindow: function(event)
  {
    ...
    window.openDialog("chrome://rtadd/content/ListDir.xul",
                      null,
                      "chrome, dialog, modal, resizable, centerscreen=yes",
                      params); 
  ...
  {    
	...
	var re1=/(\\|:|\*|\?|<|>|\||"|\.$)/g;
	var re2=document.getElementById('dir').value.replace(re1, " ");
	document.getElementById('dir').value=re2;
	var response = null;
	var re = /magnet:(.*?)/gi;                
    if ((response = re.exec(atd.torrentUrl)) != null)
    {                
        if (response[0]=="magnet:")
        {                            
				...
				if (document.getElementById('dir').value.length>0)
					atd.addTorrentUrl = atd.addTorrentUrl + '&dir_edit='+encodeURIComponent(document.getElementById('dir').value);
                ...
                var postContent = "url="+encodeURIComponent(atd.torrentUrl);
                ...
                atd.addHttpRequest.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
	...
	else
		{
			...
			atd.timer = Components.classes["@mozilla.org/timer;1"].createInstance(Components.interfaces.nsITimer);
			atd.timer.initWithCallback(event, timeout, Components.interfaces.nsITimer.TYPE_ONE_SHOT);                                                              
            atd.addHttpRequest.setRequestHeader('Referer', ""+atd.torrentUrl);
